...
class RobotStateService(
    ...
    fun pose(timeoutSeconds: Double? = null): Map<String, Any?> {
        val candidates = listOf(
            PoseCandidate(
                properties.topics.localizationPose,
                "geometry_msgs/msg/PoseWithCovarianceStamped",
                "map",
                "rtabmap_localization_pose"
            ),
    ...
    private fun currentVelocity(timeoutSeconds: Double): Map<String, Any?> {
        val odometryCandidates = listOf(
            VelocityCandidate(properties.topics.odom, "nav_msgs/msg/Odometry", "ekf_odom"),
            VelocityCandidate(properties.topics.baseOdom, "nav_msgs/msg/Odometry", "base_controller_odom"),
        )
        for (candidate in odometryCandidates) {
            val result = ros.call(
                "subscribe_once",
                mapOf(
                    "topic" to candidate.topic,
            ...
            val twist = extractOdometryTwist(msg) ?: continue
            return GatewayResult.ok(
                mapOf(
                    "topic" to candidate.topic,
                    "source" to candidate.source,
                    "frame_id" to msg.path("header", "frame_id"),
                    "child_frame_id" to msg["child_frame_id"],
                    "stamp" to msg.path("header", "stamp"),
                    "linear" to twist["linear"],
                    "angular" to twist["angular"],
                ),
            )
        }

        return GatewayResult.error("timeout", "No odometry velocity sample received within the timeout.")
    }

    private fun extractOdometryTwist(msg: Map<*, *>): Map<*, *>? {
        val twist = msg["twist"] as? Map<*, *> ?: return null
        return twist["twist"] as? Map<*, *> ?: twist
    }
    ...
    private data class VelocityCandidate(
        val topic: String,
        val messageType: String,
        val source: String,
    )
